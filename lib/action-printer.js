...
module.exports = (diffs) => {
    let formatLine = (marker, sourceFn, destFn, diff) => {
        let result =
              "[" + marker + "] " + sourceFn(diff.source.client.name + " / " + diff.source.project.name + " / " + diff.source.task.name + " (" + hoursFloatToStr(diff.time.hours) + ")")
            + "\n    => " + destFn(diff.dest.client.name + " / " + diff.dest.project.name + " / " + diff.dest.task.name + " (" + hoursFloatToStr(diff.time.hours) + ")")
    ...
    for(let dateStr in diffsByDate) {
        ...
        iDiffs.present.forEach((presentDiff) => {
            l.p(formatLine(chalk.bold.bgGreen("*"), chalk.green, chalk.gray, presentDiff))
            l.p("")
        })

        iDiffs.missing.forEach((missingDiff) => {
            l.p(formatLine(chalk.bold.bgYellow("+"), chalk.yellow, chalk.bold.yellow, missingDiff))
            l.p("")
        })
